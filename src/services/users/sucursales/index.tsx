...
const getSucursales = async (
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(`Error al obtener los sucursales: ${res.status}`);
      }

      const data = await res.json();

      const normalizedWebReportsFormats = Array.isArray(data)
        ? mapSucursalesFormatsApiToEntities(data)
        : [];
      ...
      if (attempt === maxRetries) {
        throw new Error(
          `Todos los intentos fallaron. No se pudieron obtener los  sucursales: ${error.message}`
        );
      }
    }
  }
